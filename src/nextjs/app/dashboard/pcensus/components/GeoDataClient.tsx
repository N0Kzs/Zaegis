/**
 * @file Orchestrator component for the GeoData (pcensus) module.
 *
...
import ZoningUpload from './ZoningUpload';
...
export default function GeoDataClient() {
  ...
  const navigateTab = (tab: TabId) => {
    setActiveTab(tab);
    if (tab === 'overview') {
      router.replace('/dashboard/pcensus', { scroll: false });
    } else {
      router.replace(`/dashboard/pcensus?tab=${tab}`, { scroll: false });
    }
  };
